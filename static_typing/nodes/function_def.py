...
def scan_FunctionDef(function, ast_module) -> t.Sequence[t.Tuple[str, t.Sequence[type]]]:
    """Scan FunctionDef node."""
    variables = []
    for node in ast_module.walk(function):
        if isinstance(node, ast_module.Assign):
            # TODO: restore below
            pass #variables += scan_Assign(node)
        elif isinstance(node, ast_module.AnnAssign):
            # TODO: restore below
            pass #variables += scan_AnnAssign(node)
...
def create_statically_typed_function_def(ast_module):
    ...
    class StaticallyTypedFunctionDefClass(ast_module.FunctionDef, StaticallyTyped[ast_module]):
        ...
        _type_fields = 'params', 'local_vars'
        ...
        def _add_type_info(self):
            ...
            results = scan_FunctionDef(self, ast_module)
            for k, v in results:
